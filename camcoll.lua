...
-- Config
local LINE_COLOR = Color3.new(1, 0, 0) -- red
local LINE_TRANSPARENCY = 0.5
local LINE_WIDTH = 0.1

-- Table to store beams (keyed by player)
local beams = {}
...
-- Create or get an attachment under a part
...
-- Create a beam between local player and another player
local function createBeamToPlayer(targetPlayer)
    ...
    -- Create attachments
    local att0 = getOrCreateAttachment(localRoot, 'BeamAttachment_Local')
    local att1 = getOrCreateAttachment(targetRoot, 'BeamAttachment_Target')
...
-- Update beams: remove old, create new
local function refreshBeams()
    ...
    if not LocalPlayer.Character or not getRootPart(LocalPlayer.Character) then
        return
    end

    -- For every other player
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and getRootPart(p.Character) then
            local b = createBeamToPlayer(p)
            if b then
                beams[p] = b
            end
        end
...
-- Rebuild beams when relevant events happen
...
-- Also listen for other players’ respawns
...
-- On each frame, optionally refresh to adjust beams (if you want dynamic updates)
RunService.RenderStepped:Connect(function()
    -- Optionally, you could update the beams’ attachments positions, but attachments auto move
    -- Just ensure beams exist
    refreshBeams()
end)
